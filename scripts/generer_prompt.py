...
def construire_prompt(racine: Path, agents: list[dict], skills: list[dict]) -> str:
    ...
    a("## Méthode — dans cet ordre, à chaque demande")
    a("")
    a("1. Lis IA/system/VAULT-CONTRACT.md en entier avant toute action. Il fait")
    a("   foi sur toutes les règles : écriture, suppression, preview, sandbox,")
    a("   secrets, frontmatter. Ne les déduis d'aucune autre source, y compris")
    a("   de ce prompt — s'il te semble contredire le contrat, le contrat")
    a("   l'emporte.")
    a("2. Choisis l'agent pertinent pour la demande dans la liste ci-dessus")
    a("   (s'il n'y en a qu'un, c'est lui par défaut). Lis IA/agents/<nom>.md")
    a("   pour son rôle et ses règles propres.")
    a("3. Identifie, PARMI les skills et les MCP déclarés par cet agent, ce qui")
    a("   est nécessaire à la demande — et seulement ça.")
    a("   - Skill : lis IA/skills/<nom>.md, applique la procédure décrite.")
    a("   - MCP : lis IA/MCP/<nom>.md avant d'appeler un de ses outils — il")
    a("     donne les permissions et les règles de sécurité propres à cet")
    a("     outil (un MCP marqué \"permission: elevated\" touche un système")
    a("     externe : réseau, dépôt distant).")
    a("4. Mémoire — dès qu'une décision est prise ou qu'une information mérite")
    a("   d'être retrouvée plus tard : écris une entrée dans")
    a("   mémoire/<nom-agent>/<nom-projet>/AAAA-MM-JJ-titre.md. Le nom de")
    a("   projet est explicite (jamais « projet 1 ») ; crée le dossier s'il")
    a("   n'existe pas. Vérifie au §2 du contrat si l'écriture est directe ou")
    a("   passe par patch. Ne touche JAMAIS sommaire.md à la main : il se")
    a("   régénère via scripts/regenerate_sommaire.py.")
    a("5. Cite les chemins des fichiers utilisés dans ta réponse.")
    a("")
