...
| [Rendre du Mermaid dans un conteneur demande --no-sandbox](rendu-mermaid-en-conteneur.md) | 🟢 Vérifiée le 2026-09-03 lors d'un test du workflow du… | Leçon réutilisable sur le skill `mermaid`. Vaut pour toute machine sans session graphique : conteneur, VM Debian… |
| [Un état qui ne vit que chez l'exécutant est perdu](un-etat-non-declare-est-perdu.md) | 🟢 Vérifiée — un skill agissait depuis des semaines sans… | Leçon générale, tirée du cas des tâches planifiées. Vaut pour toute configuration qu'un agent installe hors du dépôt. |
| [Une donnée partagée rangée chez un propriétaire fabrique une exception](une-donnee-partagee-rangee-chez-un-proprietaire.md) | 🟢 Vérifiée — une exception supprimée du contrat, aucune… | Leçon générale, tirée de la mémoire d'OBSIA. Vaut pour toute arborescence découpée par propriétaire — dossiers, tables… |

> Chemin dans le coffre : `mémoire/assistant/expériences/`
